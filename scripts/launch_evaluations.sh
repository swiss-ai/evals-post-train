...
#   --think-end-token T  - Token marking the end of the thinking block (only used with
#                          --enable-thinking). Default: <|inner_suffix|> (internal checkpoints).
#                          Pass 'think' as a shorthand for </think>, or any custom literal.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --think-end-token) THINK_END_TOKEN_FLAG="$2";  shift 2 ;;
...
[[ -n "$THINK_END_TOKEN_FLAG" ]] && export THINK_END_TOKEN="$THINK_END_TOKEN_FLAG"
