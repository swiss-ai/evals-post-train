...
| `--api-tokenized-requests <true\|false>` | Whether local completion requests send token ids. Defaults to `false` for Swiss-AI and official OpenAI, and `true` for other completion endpoints including local vLLM. Chat adapters always send text/messages and ignore this setting. |
...
| `API_TOKENIZED_REQUESTS` | endpoint-dependent | Local completion adapter only: send token ids (`true`) or text (`false`). Defaults to `false` for Swiss-AI and official OpenAI, and `true` otherwise. Chat adapters ignore it. |
