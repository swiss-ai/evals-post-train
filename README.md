...
| `--backend <hf\|vllm\|megatron_lm\|openai>` | Inference backend (default: from sbatch script). `vllm` recommended for in-job inference; `openai` evaluates against an already-running OpenAI-compatible endpoint instead of loading the model. |
...
| `LM_EVAL_BACKEND` | `hf` | Backend: `hf` (accelerate), `vllm`, `megatron_lm`, `openai` (OpenAI-compatible API) |
...
The sbatch scripts support `hf`, `vllm`, `megatron_lm`, and `openai` backends. To add a new one:
